...
public class CombinerChain {
    ...
    /**
     * Create a combiner chain configured for a particular subtype.
     *
     * @param initialText The text that has already been combined so far.
     * @param combiningSpec The {@code CombiningRules} extra value of the active subtype. When it
     *     names {@code "avro"}, an {@link AvroPhoneticCombiner} is added so Latin keystrokes are
     *     transliterated to Bangla.
     */
    public CombinerChain(final String initialText, final String combiningSpec) {
        mCombiners = new ArrayList<>();
        // The dead key combiner is always active, and always first
        mCombiners.add(new DeadKeyCombiner());
        if (null != combiningSpec && combiningSpec.toLowerCase().contains("avro")) {
            mCombiners.add(new AvroPhoneticCombiner());
        }
    ...
    public String getAvroRomanString() {
        for (final Combiner combiner : mCombiners) {
            if (combiner instanceof AvroPhoneticCombiner) {
                return ((AvroPhoneticCombiner) combiner).getRomanString();
            }
        }
        return "";
    }
