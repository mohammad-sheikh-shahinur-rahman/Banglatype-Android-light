...
import com.itamadersomajinc.banglatype.keyboard.internal.KeyDrawParams;
import com.itamadersomajinc.banglatype.keyboard.internal.KeyVisualAttributes;
...
public class KeyboardView extends View {
    ...
    // Custom background caching
    private Drawable mCustomBackgroundCache = null;
    private String mCustomBackgroundUriCache = "";
    private int mCustomBackgroundColorCache = 0;
    private String mCustomBackgroundTypeCache = "";
    private boolean mCustomThemeEnabledCache = false;
    ...
    private void onDrawKey(@Nonnull final Key key, @Nonnull final Canvas canvas,
        ...
        if (!key.isSpacer()) {
            ...
            final android.content.SharedPreferences prefs = android.preference.PreferenceManager.getDefaultSharedPreferences(getContext());
            if (prefs.getBoolean("pref_rgb_theme_enabled", false)) {
                final long time = android.os.SystemClock.uptimeMillis();
                final float hue = ((time / 8) + (keyDrawX / 3)) % 360f;
                final int color = android.graphics.Color.HSVToColor(new float[]{hue, 0.9f, 0.9f});
                final int originalColor = paint.getColor();
                final android.graphics.Paint.Style originalStyle = paint.getStyle();
                ...
                final android.graphics.RectF rect = new android.graphics.RectF(3, 3, keyWidth - 3, keyHeight - 3);
    ...
    protected void onDrawKeyBackground(@Nonnull final Key key, @Nonnull final Canvas canvas,
        ...
        final android.content.SharedPreferences prefs = android.preference.PreferenceManager.getDefaultSharedPreferences(getContext());
        final boolean customThemeEnabled = prefs.getBoolean("pref_custom_theme_enabled", false);
        if (customThemeEnabled) {
            final int keyColor = prefs.getInt("pref_custom_key_color", Color.DKGRAY);
            background.setColorFilter(keyColor, android.graphics.PorterDuff.Mode.MULTIPLY);
        } else {
    ...
    protected void onDrawKeyTopVisuals(@Nonnull final Key key, @Nonnull final Canvas canvas,
        ...
        if (label != null) {
            ...
            if (key.isEnabled()) {
                if (customThemeEnabled) {
                    paint.setColor(prefs.getInt("pref_custom_key_text_color", Color.WHITE));
                } else {
        ...
        if (hintLabel != null) {
            ...
            if (customThemeEnabled) {
                final int textColor = prefs.getInt("pref_custom_key_text_color", Color.WHITE);
                paint.setColor((textColor & 0x00FFFFFF) | 0x80000000); // 50% opacity
            } else {
    ...
    public Drawable getBackground() {
        ...
        final boolean customThemeEnabled = prefs.getBoolean("pref_custom_theme_enabled", false);
        if (customThemeEnabled) {
            final String bgType = prefs.getString("pref_custom_bg_type", "color");
            final int bgColor = prefs.getInt("pref_custom_background_color", Color.BLACK);
            final String uriStr = prefs.getString("pref_custom_keyboard_bg_uri", "");

            if (mCustomThemeEnabledCache == customThemeEnabled
                    && mCustomBackgroundTypeCache.equals(bgType)
                    && mCustomBackgroundColorCache == bgColor
                    && mCustomBackgroundUriCache.equals(uriStr)
                    && mCustomBackgroundCache != null) {
                return mCustomBackgroundCache;
            }

            mCustomThemeEnabledCache = customThemeEnabled;
            mCustomBackgroundTypeCache = bgType;
            mCustomBackgroundColorCache = bgColor;
            mCustomBackgroundUriCache = uriStr;

            if ("image".equals(bgType) && !uriStr.isEmpty()) {
                try {
                    ...
                    if (bitmap != null) {
                        mCustomBackgroundCache = new android.graphics.drawable.BitmapDrawable(getResources(), bitmap) {
                            @Override
                            public void draw(Canvas canvas) {
                                ...
                                if (viewWidth > 0 && viewHeight > 0) {
                                    ...
                                    float scale = Math.max((float) viewWidth / bWidth, (float) viewHeight / bHeight);
                                    int drawWidth = (int) (bWidth * scale);
                                    int drawHeight = (int) (bHeight * scale);
                                    int left = (viewWidth - drawWidth) / 2;
                                    int top = (viewHeight - drawHeight) / 2;
                                    Rect dest = new Rect(left, top, left + drawWidth, top + drawHeight);
                    ...
                    android.util.Log.e("KeyboardView", "Failed to load custom bg image: " + uriStr, e);
                }
            }
            mCustomBackgroundCache = new android.graphics.drawable.ColorDrawable(bgColor);
            return mCustomBackgroundCache;
        } else {
            mCustomBackgroundCache = null;
            mCustomThemeEnabledCache = false;
