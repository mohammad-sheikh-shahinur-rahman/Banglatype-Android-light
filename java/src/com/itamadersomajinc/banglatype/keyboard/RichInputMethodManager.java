...
import static com.itamadersomajinc.banglatype.keyboard.common.Constants.Subtype.KEYBOARD_MODE;
...
import com.itamadersomajinc.banglatype.compat.InputMethodManagerCompatWrapper;
import com.itamadersomajinc.banglatype.compat.InputMethodSubtypeCompatUtils;
...
public class RichInputMethodManager {
    ...
    /**
     * Finds a declared subtype of this IME whose keyboard layout set matches the given name
     * (e.g. "avro_phonetic"), regardless of whether it is currently enabled in the system.
     * Returns null if no such subtype is declared.
     */
    @Nullable
    public InputMethodSubtype findSubtypeByKeyboardLayoutSet(final String keyboardLayoutSetName) {
        final InputMethodInfo imi = getInputMethodInfoOfThisIme();
        final int count = imi.getSubtypeCount();
        for (int i = 0; i < count; i++) {
            final InputMethodSubtype subtype = imi.getSubtypeAt(i);
            if (keyboardLayoutSetName.equals(SubtypeLocaleUtils.getKeyboardLayoutSetName(subtype))) {
                return subtype;
            }
        }
        return null;
    ...
    public RichInputMethodSubtype getCurrentSubtype() {
        ...
        if (mContext != null) {
            final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(mContext);
            final String layoutOverride = prefs.getString("layout_override", null);
            if (layoutOverride != null) {
                final InputMethodSubtype subtype = findSubtypeByKeyboardLayoutSet(layoutOverride);
                if (subtype != null) {
                    return RichInputMethodSubtype.getRichInputMethodSubtype(subtype);
                }
            }
        }
    ...
    private void updateShortcutIme() {
        ...
        final RichInputMethodSubtype richSubtype = getCurrentSubtype();
