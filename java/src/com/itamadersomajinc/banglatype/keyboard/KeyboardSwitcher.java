...
import com.itamadersomajinc.banglatype.keyboard.KeyboardLayoutSet.KeyboardLayoutSetException;
import com.itamadersomajinc.banglatype.keyboard.emoji.EmojiPalettesView;
import com.itamadersomajinc.banglatype.keyboard.internal.KeyboardState;
import com.itamadersomajinc.banglatype.keyboard.internal.KeyboardTextsSet;
...
public final class KeyboardSwitcher implements KeyboardState.SwitchActions {
    ...
    private void setKeyboard(
        ...
        // Always pass true to support internal layout cycling indicators
        keyboardView.startDisplayLanguageOnSpacebar(subtypeChanged, languageOnSpacebarFormatType,
                true /* hasMultipleEnabledIMEsOrSubtypes */);
    }
    ...
    public View onCreateInputView(@NonNull Context displayContext,
        ...
        if (mMainKeyboardFrame != null) {
            final android.content.SharedPreferences prefs =
                    android.preference.PreferenceManager.getDefaultSharedPreferences(mThemeContext);
            final android.widget.ImageView bgImage = (android.widget.ImageView) mMainKeyboardFrame.findViewById(R.id.keyboard_background_image);
            if (bgImage != null) {
                final String imageUriStr = prefs.getString("pref_custom_keyboard_bg_uri", null);
                if (imageUriStr != null && !imageUriStr.isEmpty()) {
                    try {
                        bgImage.setImageURI(android.net.Uri.parse(imageUriStr));
            ...
            final int customBgColor = prefs.getInt("pref_custom_keyboard_bg_color", 0);
            if (customBgColor != 0 && mKeyboardView != null) {
                mKeyboardView.setBackgroundColor(customBgColor);
            }
        }
    ...
    public void setOneHandedMode(int mode) {
        mOneHandedMode = mode;
        mIsFloatingMode = false;
        updateKeyboardLayoutConstraints();
    }

    public void setFloatingMode(boolean enable) {
        mIsFloatingMode = enable;
        if (enable) {
            mOneHandedMode = 0;
        }
        updateKeyboardLayoutConstraints();
    ...
    private void updateKeyboardLayoutConstraints() {
        if (mMainKeyboardFrame == null) return;
        final View container = mMainKeyboardFrame.findViewById(R.id.keyboard_contents_container);
        ...
        final int screenWidth = mThemeContext.getResources().getDisplayMetrics().widthPixels;

        if (mIsFloatingMode) {
            lp.width = (int) (screenWidth * 0.65f);
            lp.gravity = android.view.Gravity.CENTER_HORIZONTAL | android.view.Gravity.BOTTOM;
            lp.bottomMargin = 80;
        } else if (mOneHandedMode == 1) {
            lp.width = (int) (screenWidth * 0.75f);
            lp.gravity = android.view.Gravity.START | android.view.Gravity.BOTTOM;
            lp.bottomMargin = 0;
        } else if (mOneHandedMode == 2) {
            lp.width = (int) (screenWidth * 0.75f);
            ...
            lp.gravity = android.view.Gravity.FILL_HORIZONTAL | android.view.Gravity.BOTTOM;
            lp.bottomMargin = 0;
        }
