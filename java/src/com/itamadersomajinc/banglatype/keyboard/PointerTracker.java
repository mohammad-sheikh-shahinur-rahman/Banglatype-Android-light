...
import com.itamadersomajinc.banglatype.keyboard.internal.BatchInputArbiter;
import com.itamadersomajinc.banglatype.keyboard.internal.BatchInputArbiter.BatchInputArbiterListener;
import com.itamadersomajinc.banglatype.keyboard.internal.BogusMoveEventDetector;
import com.itamadersomajinc.banglatype.keyboard.internal.DrawingProxy;
import com.itamadersomajinc.banglatype.keyboard.internal.GestureEnabler;
import com.itamadersomajinc.banglatype.keyboard.internal.GestureStrokeDrawingParams;
import com.itamadersomajinc.banglatype.keyboard.internal.GestureStrokeDrawingPoints;
import com.itamadersomajinc.banglatype.keyboard.internal.GestureStrokeRecognitionParams;
import com.itamadersomajinc.banglatype.keyboard.internal.PointerTrackerQueue;
import com.itamadersomajinc.banglatype.keyboard.internal.TimerProxy;
import com.itamadersomajinc.banglatype.keyboard.internal.TypingTimeRecorder;
...
public final class PointerTracker implements PointerTrackerQueue.Element,
    ...
    // Swipe on spacebar to change layout.
    private int mSpaceSwipeStartX;
    private int mSpaceSwipeThreshold;
    private boolean mIsSwipingOnSpace;
    private boolean mIsSpaceCursorMovementMode;
    private int mSpaceCursorAccumulatedDeltaX;
    ...
    private void setKeyDetectorInner(final KeyDetector keyDetector) {
        ...
        mSpaceSwipeThreshold = keyWidth / 2;
    ...
    private void onDownEventInternal(final int x, final int y, final long eventTime) {
        ...
        mIsSwipingOnSpace = false;
        mIsSpaceCursorMovementMode = false;
        mSpaceCursorAccumulatedDeltaX = 0;
        if (key != null && key.getCode() == Constants.CODE_SPACE) {
            mSpaceSwipeStartX = x;
        }
    ...
    private void onMoveEventInternal(final int x, final int y, final long eventTime) {
        ...
        if (mIsSpaceCursorMovementMode) {
            final int deltaX = x - lastX;
            mSpaceCursorAccumulatedDeltaX += deltaX;
            mLastX = x;
            mLastY = y;
            final int step = 20;
            while (Math.abs(mSpaceCursorAccumulatedDeltaX) >= step) {
                if (mSpaceCursorAccumulatedDeltaX > 0) {
                    sListener.onCustomRequest(Constants.CUSTOM_CODE_MOVE_CURSOR_RIGHT);
                    mSpaceCursorAccumulatedDeltaX -= step;
                } else {
                    sListener.onCustomRequest(Constants.CUSTOM_CODE_MOVE_CURSOR_LEFT);
                    mSpaceCursorAccumulatedDeltaX += step;
                }
            }
            return;
        ...
        if (oldKey != null && oldKey.getCode() == Constants.CODE_SPACE) {
            final int distanceX = x - mSpaceSwipeStartX;
            if (Math.abs(distanceX) > mSpaceSwipeThreshold) {
                mIsSwipingOnSpace = true;
            }
            mLastX = x;
            mLastY = y;
            return;
    ...
    private void onUpEventInternal(final int x, final int y, final long eventTime) {
        ...
        if (mIsSpaceCursorMovementMode) {
            final int totalMovement = Math.abs(x - mSpaceSwipeStartX);
            if (totalMovement < 10) {
        ...
        if (mIsSwipingOnSpace) {
            // Swiping right moves to the next subtype, swiping left to the previous one.
            final boolean swipingLeft = (x - mSpaceSwipeStartX) < 0;
            sListener.onCustomRequest(swipingLeft
                    ? Constants.CUSTOM_CODE_SWITCH_TO_PREV_SUBTYPE
                    : Constants.CUSTOM_CODE_SWITCH_TO_NEXT_SUBTYPE);
            return;
        }
    ...
    public void onLongPressed() {
        ...
        if (code == Constants.CODE_SPACE) {
            mIsSpaceCursorMovementMode = true;
            mSpaceCursorAccumulatedDeltaX = 0;
            if (sDrawingProxy instanceof View) {
                ((View) sDrawingProxy).performHapticFeedback(android.view.HapticFeedbackConstants.LONG_PRESS);
            }
            return;
        }
        if (code == Constants.CODE_LANGUAGE_SWITCH) {
            // Long pressing the language key invokes IME switcher dialog.
