...
package com.itamadersomajinc.banglatype.keyboard.internal;
...
import com.itamadersomajinc.banglatype.keyboard.Key;
import com.itamadersomajinc.banglatype.keyboard.Keyboard;
import com.itamadersomajinc.banglatype.keyboard.KeyboardId;
import com.itamadersomajinc.banglatype.keyboard.KeyboardTheme;
import com.itamadersomajinc.banglatype.keyboard.R;
import com.itamadersomajinc.banglatype.keyboard.common.Constants;
import com.itamadersomajinc.banglatype.keyboard.common.StringUtils;
...
public class KeyboardBuilder<KP extends KeyboardParams> {
    ...
    private void parseKeyboardAttributes(final XmlPullParser parser) {
        ...
        try {
            ...
            final float heightScale = prefs.getInt("pref_keyboard_height", 100) / 100.0f;

            final KeyboardParams params = mParams;
            final int height = (int) (params.mId.mHeight * heightScale);
            ...
            final boolean showNumberRow = prefs.getBoolean("pref_show_number_row", false) && params.mId.isAlphabetKeyboard();
            final int defaultKeyboardRows = showNumberRow ? 5 : DEFAULT_KEYBOARD_ROWS;
            params.mDefaultRowHeight = (int)ResourceUtils.getDimensionOrFraction(keyboardAttr,
                    R.styleable.Keyboard_rowHeight, baseHeight, baseHeight / defaultKeyboardRows);
    ...
    private void addNumberRowIfNecessary() {
        final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(mContext);
        final boolean showNumberRow = prefs.getBoolean("pref_show_number_row", false) && mParams.mId.isAlphabetKeyboard();
        if (!showNumberRow) {
            return;
        }

        final KeyboardRow row = new KeyboardRow(mParams, mCurrentY);
        startRow(row);

        final boolean isBengali = "bn".equals(mParams.mId.getLocale().getLanguage());
        final String[] labels = isBengali
                ? new String[] {"১", "২", "৩", "৪", "৫", "৬", "৭", "৮", "৯", "০"}
                : new String[] {"1", "2", "3", "4", "5", "6", "7", "8", "9", "0"};
        final int[] codes = isBengali
                ? new int[] {'১', '২', '৩', '৪', '৫', '৬', '৭', '৮', '৯', '০'}
                : new int[] {'1', '2', '3', '4', '5', '6', '7', '8', '9', '0'};

        final float keyWidth = mParams.mBaseWidth / 10.0f;
        final int labelFlags = row.getDefaultKeyLabelFlags();
        final int backgroundType = row.getDefaultBackgroundType();
        final int height = row.getRowHeight();
        ...
        for (int i = 0; i < 10; ++i) {
            final int x = (int)row.getKeyX(null);
            final int y = row.getKeyY();
            final Key key = new Key(labels[i], KeyboardIconsSet.ICON_UNDEFINED, codes[i], null /* outputText */,
                    null /* hintLabel */, labelFlags, backgroundType, x, y, (int)keyWidth, height,
                    mParams.mHorizontalGap, mParams.mVerticalGap);
            endKey(key);
            row.advanceXPos(keyWidth);
        }
