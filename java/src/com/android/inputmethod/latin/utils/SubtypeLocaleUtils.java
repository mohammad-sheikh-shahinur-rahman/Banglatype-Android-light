...
import static com.itamadersomajinc.banglatype.keyboard.common.Constants.Subtype.ExtraValue.COMBINING_RULES;
import static com.itamadersomajinc.banglatype.keyboard.common.Constants.Subtype.ExtraValue.KEYBOARD_LAYOUT_SET;
import static com.itamadersomajinc.banglatype.keyboard.common.Constants.Subtype.ExtraValue.UNTRANSLATABLE_STRING_IN_SUBTYPE_NAME;
...
public final class SubtypeLocaleUtils {
    ...
    // The package that owns the app resources. NOTE: this may differ from the R class /
    // namespace package when applicationId != namespace, so it must be read from the
    // resource table at runtime (see initLocked) rather than from R.class.getPackage().
    ...
    private static void initLocked(final Context context) {
        ...
        // Resolve the real resource package from the resource table (applicationId may differ
        // from the namespace/R package), otherwise getIdentifier() returns 0 and crashes.
        final String RESOURCE_PACKAGE_NAME = res.getResourcePackageName(R.string.subtype_generic);
